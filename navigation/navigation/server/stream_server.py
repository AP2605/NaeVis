...
Central communication hub connecting Blender (P2), Navigation (P3), and Dashboard (P4):
  ...
  - Supports both standard WebSocket connections and HTTP REST polling.
...
class NavigationStreamServer:
    ...
    async def register_telemetry_client(self, websocket):
        ...
        print(f"[Server] P4 Dashboard connected: {websocket.remote_address} (Total: {len(self.telemetry_clients)})")
        try:
    ...
    async def handle_sensor_feed(self, websocket):
        ...
        print(f"[Server] P2 Blender connected: {websocket.remote_address}")
        try:
            async for message in websocket:
                try:
                    ...
                    if self.frame_counter % 30 == 0:
                        ...
                        print(f"[Server] Frame #{self.frame_counter:04d} | Pose: ({pose['x']:.2f}, {pose['y']:.2f}, {pose['z']:.2f}) m | Latency: {dt_ms:.1f}ms | Dashboard Clients: {len(self.telemetry_clients)}")
    ...
    async def router(self, websocket, path):
        """Routes incoming WebSocket connections based on URL path."""
        if path == "/ws/telemetry" or path == "/":
            # P4 Dashboard endpoint
            await self.register_telemetry_client(websocket)
        elif path == "/ws/sensors":
            # P2 Blender simulator endpoint
    ...
    async def start(self):
        ...
        print(f"  • P2 Blender Feed URL:    ws://localhost:{self.port}/ws/sensors")
        print(f"  • P4 Dashboard Stream URL: ws://localhost:{self.port}/ws/telemetry")
        print("=" * 65 + "\n")
